        ...
        <div class="col-xl-12">
            <div class="card">
                ...
                <div>
                    ...
                    <a class="btn btn-primary"  style="padding: 10px 20px; margin: 10px;"  href="{{ route('competitions.create')}}"><i class="fa fa-plus fa-lg" aria-hidden="true"></i> Add Competition </a>
                    <br>
                </div>
                            ...
                            <table class="table" id="resultTable">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Start date</th>
                                        <th>End date</th>
                                        <th>Length of sprint</th>
                                        <th>No of free token</th>
                                        <!-- <th>Expiry date of token</th> -->
                                        <th>Rule of competition</th>
                                        @if($admin)
                                        <th>Actions</th>
                                        @endif
                                    </tr>
                                ...
                                <tbody>
                                    ...
                                    @php $status = 0; @endphp
                                    @foreach($res->participant as $key => $row)

                                    @if($row->competition_id==$res->id)
                                    @php
                                    $status = $row->status; @endphp
                                    @endif
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $res->name }} </td>
                                        <td>{{ date('Y-m-d', strtotime($res->startdate)) }}</td>
                                        <td>{{ date('Y-m-d', strtotime($res->enddate)) }}</td>
                                        <td>{{ $res->sprint }}</td>
                                        <td>{{ $res->token_no }}</td>
                                        <td>
                                            <p class="content-description">{!! $res->rule_of_competition !!} </p>
                                        </td>
                                        ...
                                        <td>
                                            ...
                                            Edit
                                                ...
                                                <a href="{{route('competitions.show',$res->id)}}" class="btn btn-primary">
                                                    Edit
                                                </a>
                                                ...
                                                <!-- Delete Button -->
                                                <form action="{{ route('competitions.destroy', $res->id) }}" method="Post" style="display: inline;">
                                                    @csrf
                                                    @method('DELETE')
                                    ...
                                    <div class="modal fade" id="editpujaModal{{ $res->id }}" tabindex="-1" aria-labelledby="editpujaModalLabel{{ $res->id }}" aria-hidden="true">
                                        <div class="modal-dialog dialogue-width modal-xl">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <form id="editForm{{ $res->id }}" method="POST" action="{{ route('competitions.update', $res->id) }}" enctype="multipart/form-data">
                                                        @csrf
                                                        @method('PATCH')
                                                        <div class="row">
                                                            <div class="col-md-6 mb-2">
